...
#define TAKE_5BITS(x) (((uint8_t)x) & right5_mask8)

// some constants
static uint8_t right5_mask8 = ((uint8_t) ~0) >> 3;
...
// 2.4 Key Schedule
static void mk_subkeys(uint32_t *mask_keys, uint8_t *rot_keys, uint64_t *key) {
...
// function Type 2
static uint32_t f2(uint32_t D, uint32_t km, uint8_t kr) {
    // I = ((Kmi ^ D) <<< Kri)
    uint32_t I = leftrot(km ^ D, kr);
    uint8_t Ia = ((uint8_t*) &I)[0];
    uint8_t Ib = ((uint8_t*) &I)[1];
...
// function Type 3
static uint32_t f3(uint32_t D, uint32_t km, uint8_t kr) {
    // I = ((Kmi - D) <<< Kri)
    uint32_t I = leftrot(km - D, kr);
    uint8_t Ia = ((uint8_t*) &I)[0];
    uint8_t Ib = ((uint8_t*) &I)[1];
...
static void do_round(uint32_t (*f)(uint32_t, uint32_t, uint8_t),
